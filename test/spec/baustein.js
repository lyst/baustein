...
import baustein from '../../src/baustein';
...
describe('baustein', function () {
    ...
    function triggerNodeInserted(node) {
        for (let i = 0; i < document.body.addEventListener.callCount; i++) {
            const call = document.body.addEventListener.getCall(i);
            if (call.args[0] === 'DOMNodeInserted') {
                call.args[1]({target: node, type: 'DOMNodeInserted'});
                return;
            }
        }
    }

    function triggerNodeRemoved(node) {
        for (let i = 0; i < document.body.addEventListener.callCount; i++) {
            const call = document.body.addEventListener.getCall(i);
            if (call.args[0] === 'DOMNodeRemoved') {
                call.args[1]({target: node, type: 'DOMNodeRemoved'});
                return;
            }
        }
    }
    ...
    describe('baustein.Component', function () {
        ...
        describe('#onInsert', function () {
            ...
            it('should be called when the component is inserted into the DOM', function () {
        ...
        describe('#onRemove', function () {
            ...
            it('should be called when the component is removed from the DOM', function () {
                ...
                c1.appendTo(parent);
                triggerNodeInserted(c1.el);

                c2.appendTo(c1);
                triggerNodeInserted(c2.el);

                document.body.removeChild(parent);
                triggerNodeRemoved(parent);
